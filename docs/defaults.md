...
- `arb.int(min, max)`: Default is `min` for positive ranges, `-1` for negative
  ranges, or `0` if the range spans zero.
- `arb.boolean()`: Default is `false`.
- `arb.string()` and other string arbitraries: Default is the empty string `""`.
- `arb.array(...)`: Default is an empty array `[]`, unless a fixed length is
  specified.
