...
export default class Arbitrary<T> implements PickSet<T> {
  ...
  findGenerated(
    ...
    for (const gen of this.generateAll()) {
      ...
      if (++count >= limit) {
        throw new Error(
          `No ${this.label} values matched within ${limit} tries`,
        );
      }
    }
    return undefined;
  ...
  takeAll(opts?: { limit?: number }): T[] {
    ...
    if ((examples.length > limit)) {
      throw new Error(
        `wanted at most ${limit} values from ${this.label}, got more`,
      );
    }
    return examples;
  }
  ...
  filter(
    ...
    if (!accept(this.default().val)) {
      ...
      if (gen === undefined) {
        throw new Error(
          `filter didn't accept any values generated by ${this.label}`,
        );
      }
