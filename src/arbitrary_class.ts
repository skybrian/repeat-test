...
export interface PickSet<T> {
  get innerPick(): (pick: PickFunction, picker?: PlayoutPicker) => T;
}
...
export function makePickFunction<T>(
  ...
  const dispatch = <T>(
    ...
    const innerPick = req["innerPick"];
    if (typeof innerPick === "function") {
      const accept = opts?.accept;
      if (accept === undefined) {
        return innerPick(dispatch, picker);
      }
      ...
      while (true) {
        ...
        const val = innerPick(dispatch, picker);
...
export default class Arbitrary<T> implements PickSet<T> {
  ...
  static from<T>(
    callback: ArbitraryCallback<T> | PickSet<T> | T[],
    opts?: { label?: string },
  ): Arbitrary<T>;
  static from<T>(
    arg: PickRequest | ArbitraryCallback<T> | PickSet<T> | T[],
    opts?: { label?: string },
    ...
    const innerPick = arg["innerPick"];
    if (typeof innerPick === "function") {
      const label = opts?.label ?? "(unlabeled PickSet)";
      return new Arbitrary(label, arg.innerPick);
    }
